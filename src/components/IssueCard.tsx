...
function PRBadge({ s }: { s: PRStatus }) {
  ...
  if (s.status === "guest") return (
    <span style={{ background: "var(--gt-card-hover)", color: "var(--gt-text-subtle)", borderColor: "var(--gt-border)" }} className={base}>
  ...
  if (s.status === "safe") return (
    <span style={{ background: "var(--gt-safe-bg)", color: "var(--gt-safe-text)", borderColor: "var(--gt-safe-border)" }} className={base}>
      ✓ Safe to claim
    </span>
  );
  if (s.status === "open_pr") return (
    <span style={{ background: "var(--gt-danger-bg)", color: "var(--gt-danger-text)", borderColor: "var(--gt-danger-border)" }} className={base}>
      Active PR exists
    </span>
  );
  if (s.status === "draft_pr") return (
    <span style={{ background: "var(--gt-warn-bg)", color: "var(--gt-warn-text)", borderColor: "var(--gt-warn-border)" }} className={base}>
      Draft PR exists
    </span>
  );
