...
const PUBLIC_SPONSOR_QUERY = `
  query PublicSponsor($login: String!) {
    user(login: $login) {
      sponsoring {
        totalCount
      }
...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch("https://api.github.com/graphql", {
      ...
      body: JSON.stringify({ query: PUBLIC_SPONSOR_QUERY, variables: { login: username } }),
    });
    ...
    const count = data?.user?.sponsoring?.totalCount ?? 0;
    return NextResponse.json({ isEarned: count > 0, sponsoringCount: count });
  } catch (err) {
    console.error("[public-sponsor]", err);
